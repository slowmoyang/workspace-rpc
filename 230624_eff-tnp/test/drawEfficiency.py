...
def plot_geom(df_eff: pd.DataFrame,
    ...
    for _, row in df_eff.iterrows():
        det_id = RPCDetId.from_name(row.roll_name)
        try:
            key, bin = rpc_shapes.idToBin[det_id]
        except:
            print(f'RPCDetId not found from {det_id}')
        # FIXME
        if row.denominator <= min_denominator:
            # TODO warnings.warn
            # warnings.warn(f"{det_id} does not pass the minimum denominator.")
            continue
...
def main():
    ...
    parser.add_argument('-o', '--output-dir', default=(Path.cwd() / 'data' / 'step3_efficiency'), type=Path, help='Help text')
